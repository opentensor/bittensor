...
class RegisterSubnetworkCommand:
    ...
    def _run(cli: "bittensor.cli", subtensor: "bittensor.subtensor"):
        ...
        table.add_column(
            f"Recieved({unit})", style="green", no_wrap=True, justify="right"
        )
        table.add_column(
            f"Locked ({unit})", style="light_goldenrod2", no_wrap=True, justify="center"
        )
        table.add_column(
        ...
        with bittensor.__console__.status(
            f":satellite: Registering subnet {netuid}..."
        ):
    ...
    def check_config(cls, config: "bittensor.config"):
        ...
        if not config.is_set("wallet.hotkey") and not config.no_prompt:
            wallet_hotkey = Prompt.ask(
                "Enter wallet [light_salmon3]hotkey[/light_salmon3] (to register on the network at creation)",
