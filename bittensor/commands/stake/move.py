...
class MoveStakeCommand:
    ...
    def add_args(cls, parser: argparse.ArgumentParser):
        stake_parser = parser.add_parser("move", help="""Move stake from one hotkey to another on different subnets.""")
        stake_parser.add_argument("--origin_netuid", dest="origin_netuid", type=int, required=False)
        stake_parser.add_argument("--destination_netuid", dest="destination_netuid", type=int, required=False)
        stake_parser.add_argument("--origin_hotkey", required=False, type=str, help="""Specify the origin hotkey by name or ss58 address.""")
        stake_parser.add_argument("--destination_hotkey", required=False, type=str, help="""Specify the destination hotkey by name or ss58 address.""")
        ...
        stake_parser.add_argument("--no_prompt", "--y", "-y", dest = 'no_prompt', required=False, action='store_true', help="""Specify this flag to move stake without prompt""" )
    ...
    def run(cli: "bt.cli"):
        ...
        if not destination_netuid:
            if not config.no_prompt:
                destination_netuid = int(Prompt.ask("Enter destination netuid", default="0"))
            else:
        ...
        origin_stake_balance: bt.Balance = subtensor.get_stake_for_coldkey_and_hotkey_on_netuid(
        ...
        if config.get("amount"):
            ...
            if Confirm.ask(f"Move all: [bold]{origin_stake_balance}[/bold]?"):
                ...
                try:
                    amount = float(Prompt.ask(f"Enter amount to move in {bt.Balance.get_unit( origin_netuid )}"))
        ...
        if not config.no_prompt:
            ...
            table.add_row(
                ...
                bt.Balance.get_unit(destination_netuid) + "(" + str(destination_netuid) + ")",
