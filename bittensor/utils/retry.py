...
def _retry_enabled() -> bool:
    return os.environ.get("BT_RETRY_ENABLED", "False").lower() in ("true", "1", "yes", "on")
...
def retry_call(
    func: Callable,
    *args,
    retry_exceptions: Union[Type[Exception], Tuple[Type[Exception], ...]] = (OSError, TimeoutError),
    ...
    for attempt in range(1, _max_attempts + 1):
        try:
            ...
            if attempt == _max_attempts:
                logger.debug(f"Retry exhausted after {_max_attempts} attempts. Last error: {e}")
                raise e
            
            backoff = _get_backoff_time(attempt - 1, _base_delay, _max_delay)
            logger.debug(f"Retry attempt {attempt}/{_max_attempts} failed with {e}. Retrying in {backoff:.2f}s...")
            time.sleep(backoff)
