...
def solve_cuda(nonce_start: np.int64, update_interval: np.int64, TPB: int, block_and_hotkey_hash_bytes: bytes, bn: int, difficulty: int, hotkey_bytes: bytes, limit: int, dev_id: int = 0) -> Tuple[np.int64, bytes]:
    ...
    solution = cubit.solve_cuda(TPB, nonce_start, update_interval, upper_bytes, block_and_hotkey_hash_bytes, dev_id) # 0 is first GPU
    seal = None
    if solution != -1:
        seal = create_seal_hash(block_and_hotkey_hash_bytes, solution)
