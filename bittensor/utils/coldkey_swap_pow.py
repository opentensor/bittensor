...
def _solve_for_coldkey_swap_difficulty_cpu(
    ...
    if update_interval is None:
        update_interval = 50_000_000 # Can be very high, no age criteria

    limit = int(math.pow(2, 256)) - 1

    curr_block, curr_block_num, curr_diff = _create_shared_memory()
...
def _solve_for_coldkey_swap_difficulty_cuda(
    ...
    block_and_coldkey_hash = _create_block_and_coldkey_hash(block_bytes, coldkey_bytes)
    ...
    try:
        while iteration_count < max_iterations:
            ...
            if new_block_number != block_number:
                ...
                block_bytes = bytes.fromhex(block_hash[2:])
                block_and_coldkey_hash = _create_block_and_coldkey_hash(
...
def _get_swap_difficulty_with_retry(subtensor, coldkey_address):
    max_retries = 3
    for _ in range(max_retries):
        try:
...
def _update_curr_block(
    curr_diff, curr_block, curr_block_num, block_number, block_bytes, coldkey_bytes, check_block, difficulty
):
    with check_block:
        curr_block_num.value = block_number
        curr_block[:32] = block_bytes
        curr_block[32:] = coldkey_bytes

        # Update difficulty
        curr_diff.value = difficulty
...
def _create_block_and_coldkey_hash(block_bytes: bytes, coldkey_bytes: bytes) -> bytes:
    return block_bytes + coldkey_bytes
    ...
    def run(self):
        while not self.stopEvent.is_set():
            ...
            with self.check_block:
                ...
                coldkey_bytes = bytes(self.curr_block[32:])
