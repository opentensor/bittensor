...
class server(torch.nn.Module):
    ...
    def check(self):
        ...
        """
        if not (self.tokenizer.name_or_path == self.pre_model.name_or_path): raise(ValueError('incorrect model ({}) and tokenizer ({})'.format(self.pre_model.name_or_path,self.tokenizer.name_or_path)))
        if self.interpolate == False:
            if not (self.mapping_function != None): raise(ValueError('Incorrect Settings; needs atleast one mapping function for sequence length changes'))
