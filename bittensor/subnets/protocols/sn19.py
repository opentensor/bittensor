...
class GenerateImagesFromText(bt.Synapse):
    ...
    samples: int = Field(
        cst.DEFAULT_SAMPLES, description="Number of sample images to generate"
    )
    steps: int = Field(
        cst.DEFAULT_STEPS, description="Number of steps in the image generation process"
    )
    style_preset: str = Field(
        cst.DEFAULT_STYLE_PRESET, description="Preset style for the image"
    )
    seed: int = Field(
        default=random.randint(1, cst.LARGEST_SEED),
        description="Random seed for generating the image",
    )

    image_b64s: Optional[List[str]] = Field(
        None, description="The base64 encoded images to return", title="image_b64s"
    )
...
class GenerateImagesFromImage(bt.Synapse):
    ...
    sampler: Optional[str] = Field(
        None, description="The sampler to use for image generation"
    )
    style_preset: str = Field(
        cst.DEFAULT_STYLE_PRESET, description="Preset style for the image"
    )
    ...
    init_image_mode: Optional[str] = Field(
        "IMAGE_STRENGTH", description="The mode of the init image"
    )
...
class GenerateImagesFromInpainting(bt.Synapse):
    ...
    mask_source: Optional[MaskSource] = Field(
...
class ClipEmbeddingImages(bt.Synapse):
    ...
    image_b64s: Optional[List[str]] = Field(
        None, description="The base64 encoded images", title="images"
    )
    ...
    def check_total_image_size(cls, values):
        if values is not None:
            ...
            if image_b64s:
                total_size_mb = sum(
                    (len(base64.b64decode(img)) for img in image_b64s)
                ) / (1024 * 1024)
            if total_size_mb > max_size_mb:
                raise ValueError(
                    f"Total image size should not exceed {max_size_mb} MB, we are not made of bandwidth"
                )
        return values
...
class ClipEmbeddingTexts(bt.Synapse):
    text_prompts: Optional[List[str]] = Field(
        None, description="The text prompts", title="text_prompts"
    )
...
class SegmentingSynapse(bt.Synapse):
    ...
    image_uuid: Optional[str] = Field(
        None, description="The UUID for the image to be segmented", title="embedding"
    )

    image_b64: Optional[str] = Field(
        None, description="The base64 encoded image", title="image"
    )
    error_message: Optional[str] = Field(
        None,
    ...
    def deserialize(
        self,
    ) -> Tuple[Optional[List[List[List[int]]]], Optional[List[int]]]:
        """
