...
class GPT2LMSynapse(bittensor.Synapse):
    ...
    def __init__(self, config: GPT2Config):
        ...
        self.joiner = torch.nn.Linear(2 * bittensor.__network_dim__, bittensor.__network_dim__)
    ...
    def forward( self, inputs: torch.LongTensor, query: bool = False):
        ...
        if query:
            ...
            requests, _ = self.router.route( synapses, local_pooled, inputs ) # routes inputs to network.
        ...
        student_encoding = self.student_transformer(input_ids=inputs, return_dict = True)
        if query:
            # Distillation loss between student_pooled and network inputs.
            distillation_loss = F.mse_loss(student_encoding.last_hidden_state, network_encoding) 
            loss = loss + distillation_loss

        # Join student and local embedding.
        local_output = self.joiner ( torch.cat([local_encoding.last_hidden_state, student_encoding.last_hidden_state], dim = 2)) 
