...
class BertNSPSynapse(bittensor.Synapse):
    ...
    def forward(    self, 
        ...
        # Run local and student models.
        local_encoding = self.transformer(inputs, attention_mask=attention_mask, return_dict=True)
        ...
        # Distillation model.
        student_encoding = self.student(inputs, attention_mask=attention_mask, return_dict=True).last_hidden_state
        if query:
            # Distillation loss between student_pooled and network inputs.
            distillation_loss = F.mse_loss(student_encoding, network_encoding) 
            loss = loss + distillation_loss
