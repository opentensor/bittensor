...
from bittensor.asyncbt.utils import async_substrate
...
async def get_delegated(ss58: str, rpc: async_substrate.AsyncSubstrateInterface, block_hash=None):
    encoded = subtensor_module.ss58_to_vec_u8(ss58)
    result = await rpc.rpc_request("delegateInfo_getDelegated", [encoded], reuse_block_hash=True)
    return result
...
async def root_list(block_hash, rpc: async_substrate.AsyncSubstrateInterface):
    ...
    json_result = await rpc.rpc_request(
        "state_call", ["NeuronInfoRuntimeApi_get_neurons_lite", "0x0000"], reuse_block_hash=True
    )
...
async def get_balance(block_hash, address: str, rpc: async_substrate.AsyncSubstrateInterface):
    result = await rpc.query_multiple([address], "Account", "System", reuse_block_hash=True)
