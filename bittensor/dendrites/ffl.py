...
class PKMDispatcher():
    r""" Tool for splitting and combining inputs based on gating scores.
    """
    def dispatch(self, inputs: torch.FloatTensor, gates: torch.FloatTensor) -> torch.FloatTensor:
        r""" Dispatches inputs to a set of requests for each uid based on scores.
            Args:
                inputs (:obj:`torch.FloatTensor` of shape :obj:`(batch_size, **input.shape[1:])`, `required`): 
                    inputs to dispatch.

                gates (:obj:`torch.FloatTensor` of shape :obj:`(batch_size, n_uids)`, `required`): 
                    gating values used to combine responses.

            Returns: 
                requests (:obj:`List[torch.FloatTensor]` of shape :obj:`n_uids * [(-1, **input.shape[1:])]`, `required`): 
                    dispatched examples.
        """
    ...
    def combine(self, responses: List[torch.FloatTensor], gates: torch.FloatTensor) -> torch.FloatTensor:
        r""" Combines responses from neurons based on passed gates.
            Args:
                responses (:obj:`List[torch.FloatTensor]` of shape :obj:`n_uids * (-1, sequence_dim, bittensor.__network_dim)`, `required`): 
                    responses from neurons to combine.
        ...
        gates_expanded = gates[self.batch_index.flatten()]
        ...
        nonzero_gates = torch.gather(gates_expanded, 1, self.uids_index)
        ...
        combined = zeros.index_add(0, self.batch_index, flat_stitched.float())
        ...
        combined = combined.view(batch_size, responses[0].shape[1], bittensor.__network_dim__)

        # Return.
        return combined

class PKMKeys(nn.Module):
    r""" Tool for mapping between unique identifiers and torch keys.
    """
    ...
    def forward(self, uids: torch.Tensor) -> torch.Tensor:
        ...
        if max_uid > self._n_keys:
            new_keys = torch.rand(max_uid - self._n_keys, self._key_dim)
...
class PKMDendrite():
    def __init__(self, config, session, query_dim):
        ...
        # (Inputs, Scores) -> Requests
        # (Responses, Scores) -> Outputs
        self.dispatcher = PKMDispatcher()
        # Query -> Keys
        self.projection = nn.Linear(query_dim, self.config.dendrite.key_dim, bias=True)

    def _route(self, inputs: torch.FloatTensor, query: torch.FloatTensor, modality: bittensor_pb2.Modality):
        ...
        scores, gates = self._score_uids (filtered_uids, query)

        # requests: List(torch.FloatTensor): examples for each uids
        # requests.shape = n_uids * [-1, inputs.shape[1:]]
        requests = self.dispatcher.dispatch(inputs, gates)      
        ...
        # combined: (torch.FloatTensor): combined responses reshaped to correct dimension.
        # combined = [batch_size, sequence_dim, bittensor.__network_dim__]
        combined = self.dispatcher.dispatch(responses, gates)
    ...
    def _filter_uids(self, uids: torch.LongTensor) -> torch.LongTensor:
        r""" Filters uids before scoring and dispatching.
            Args:
                uids (:obj:`torch.LongTensor` of shape :obj:`(n)`, `required`): 
                    uids to filter based on emission block.
            Returns: 
                filtered_uids (:obj:`torch.LongTensor` of shape :obj:`(n)`, `required`): 
                    fitlered uids.
        """
    ...
    def _score_uids(self, uids: torch.LongTensor, query: torch.FloatTensor) -> torch.FloatTensor:
        r""" Returns a score for each uid for each example based on queries.
            Args:
                uids (:obj:`torch.LongTensor` of shape :obj:`(n)`, `required`): 
                    uids to filter based on emission block.
            ...
            Returns: 
                scores (:obj:`torch.FloatTensor` of shape :obj:`(batch_size, n_uids)`, `required`): 
                    scores for each uid for each example.
        """
        ...
        real_topk = min( uids.shape[0], self.config.dendrite.topk )
        ...
        zeros = torch.zeros(query.shape[0], uids.shape[0])
