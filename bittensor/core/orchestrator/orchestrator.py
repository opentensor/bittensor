...
class OptimisticNonceManager:
    ...
    async def sync(self):
        ...
        async with self._lock:
            ...
            self.local_nonce = await self.subtensor.get_account_nonce(
                self.wallet_address
            )
...
class TransactionOrchestrator:
    ...
    def __init__(
        ...
        self.nonce_manager = OptimisticNonceManager(
            self.subtensor, self.wallet.hotkey.ss58_address
        )
        self.queue: asyncio.PriorityQueue = (
            asyncio.PriorityQueue()
        )  # Stores TransactionMetadata
    ...
    async def submit_extrinsic(
        ...
        logging.info(
            f"Queued transaction {call_module}.{call_function} with nonce {tx.assigned_nonce}"
        )
                ...
                try:
                    ...
                    try:
                        ...
                        tx_hash = (
                            ...
                            if hasattr(receipt, "extrinsic_hash")
                            else str(receipt)
                        )
                        ...
                        logging.info(
                            f"Submitted {tx.id} (Nonce: {tx.assigned_nonce}) Hash: {tx_hash}"
                        )
                        ...
                        if "1014" in error_str or "Priority is too low" in error_str:
                            logging.warning(
                                f"Priority too low for {tx.id}. Triggering RBF."
                            )
                            await self._trigger_rbf(tx)
                        elif (
                            "Invalid Nonce" in error_str
                            or "Transaction is outdated" in error_str
                        ):
                            logging.error(
                                f"Nonce misalignment for {tx.id}. Triggering re-sync."
                            )
                ...
                for tx in list(self.active_transactions.values()):
                    if tx.status == TransactionStatus.SUBMITTED:
                        ...
                        if current_time - tx.last_submit_at > self.config.stuck_timeout:
                            logging.warning(
                                f"Transaction {tx.id} stuck. Triggering RBF."
                            )
                            await self._trigger_rbf(tx)
                        else:
