...
async def add_stake_burn_extrinsic(
    ...
    *,
    ...
    try:
        ...
        call = await SubtensorModule(subtensor).add_stake_burn(
            netuid=netuid,
            hotkey=hotkey_ss58,
            amount=amount.rao,
...
async def add_stake_multiple_extrinsic(
    ...
    *,
    ...
    Note:
        ...
        In the key, `idx` is the index the stake attempt.

        This allows the caller to inspect which specific operations were attempted and which were not.
    """
    try:
        if not (
            unlocked := ExtrinsicResponse.unlock_wallet(wallet, raise_error)
        ...
        all_stakes = await subtensor.get_stake_info_for_coldkey(
        ...
        for idx, (hotkey_ss58, amount, old_stake, netuid) in enumerate(
            ...
            try:
                ...
                response = await add_stake_extrinsic(
                    subtensor=subtensor,
                    wallet=wallet,
                    netuid=netuid,
                    hotkey_ss58=hotkey_ss58,
...
async def set_auto_stake_extrinsic(
    ...
    *,
    ...
    try:
        ...
        call = await SubtensorModule(subtensor).set_coldkey_auto_stake_hotkey(
