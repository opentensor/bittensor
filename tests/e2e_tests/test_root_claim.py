...
def test_get_root_alpha_dividends_per_subnet(
    subtensor, alice_wallet, bob_wallet, charlie_wallet
):
    """Tests get_root_alpha_dividends_per_subnet method.
    ...
    - Set claim type to KeepSubnets([sn2.netuid]) for staker CK
    - Stake to Alice in ROOT
    - Verify that get_root_alpha_dividends_per_subnet returns correct Balance with unit set to netuid
    - Verify that the value increases over epochs
    """
    TEMPO_TO_SET = 20 if subtensor.chain.is_fast_blocks() else 10
    ...
    # Get initial root alpha dividends
    initial_dividends = subtensor.staking.get_root_alpha_dividends_per_subnet(
        hotkey_ss58=alice_wallet.hotkey.ss58_address,
        netuid=sn2.netuid,
    )

    # Verify Balance has correct netuid set
    assert initial_dividends.netuid == sn2.netuid, (
        f"Balance netuid should be {sn2.netuid}"
    )
    ...
    # Check that root alpha dividends increased
    final_dividends = subtensor.staking.get_root_alpha_dividends_per_subnet(
        hotkey_ss58=alice_wallet.hotkey.ss58_address,
        netuid=sn2.netuid,
    )

    assert final_dividends >= initial_dividends, (
        f"Root alpha dividends should increase: {final_dividends} >= {initial_dividends}"
    )
    assert final_dividends.netuid == sn2.netuid, (
        f"Balance netuid should remain {sn2.netuid}"
    )


@pytest.mark.asyncio
async def test_get_root_alpha_dividends_per_subnet_async(
    async_subtensor, alice_wallet, bob_wallet, charlie_wallet
):
    """Tests get_root_alpha_dividends_per_subnet method.
    ...
    # Get initial root alpha dividends
    initial_dividends = await async_subtensor.staking.get_root_alpha_dividends_per_subnet(
        hotkey_ss58=alice_wallet.hotkey.ss58_address,
        netuid=sn2.netuid,
    )
    ...
    # Check that root alpha dividends increased
    final_dividends = await async_subtensor.staking.get_root_alpha_dividends_per_subnet(
        hotkey_ss58=alice_wallet.hotkey.ss58_address,
        netuid=sn2.netuid,
    )
