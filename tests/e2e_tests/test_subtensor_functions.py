...
async def test_subtensor_extrinsics(subtensor, templates, alice_wallet, bob_wallet):
    ...
    assert await subtensor.get_subnets() == [0, 1]
    assert await subtensor.get_total_subnets() == 2

    # Assert correct balance is fetched for Alice
    alice_balance = await subtensor.get_balance(alice_wallet.coldkeypub.ss58_address)
    assert (
    ...
    assert await subtensor.get_subnets() == [0, 1, 2]
    assert await subtensor.get_total_subnets() == 3

    # Verify subnet 2 created successfully
    assert await subtensor.subnet_exists(netuid)

    # Default subnetwork difficulty
    assert (
        await subtensor.difficulty(netuid) == 10_000_000
    ), "Couldn't fetch correct subnet difficulty"

    # Verify Alice is registered to netuid 2 and Bob isn't registered to any
    assert await subtensor.get_netuids_for_hotkey(
    ...
    bob_balance = await subtensor.get_balance(bob_wallet.coldkeypub.ss58_address)
    ...
    neuron_info_old = await subtensor.get_neuron_for_pubkey_and_subnet(
    ...
    # neuron_info = await subtensor.get_neuron_for_pubkey_and_subnet(
    #     alice_wallet.hotkey.ss58_address, netuid=netuid
    # )
    # assert (
    ...
    assert (
        await subtensor.get_existential_deposit() == existential_deposit
    ), "Existential deposit value doesn't match with pre-defined value"

    # Fetching all subnets in the network
    all_subnets = await subtensor.get_all_subnets_info()
