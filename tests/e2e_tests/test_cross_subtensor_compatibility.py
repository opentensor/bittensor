...
async def test_get_timestamp(subtensor, async_subtensor, local_chain):
    with subtensor:
        ...
        assert isinstance(
            subtensor.chain.get_timestamp(), datetime
        )  # verify it works with no block number specified
        sync_result = subtensor.chain.get_timestamp(
            block=block_number
        )  # verify it works with block number specified
    async with async_subtensor:
        assert isinstance(await async_subtensor.chain.get_timestamp(), datetime)
        async_result = await async_subtensor.chain.get_timestamp(block=block_number)
