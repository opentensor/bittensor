...
class TestCifarSwarm(unittest.TestCase):
    num_nodes = 2
    ...
    def setUp(self):
        ...
        for i in range(self.num_nodes):
            ...
            synapse = DPNSynapse(model_config, dendrite, meta)
    ...
    def test_cifar_swarm_loss(self):
        logger.info('Load CIFAR dataset.')
        batch_size_train = 64
        train_data = torchvision.datasets.CIFAR10(root = self.configs[0].datapath + "datasets/", train=True, download=True, transform=transforms.ToTensor())
        trainloader = torch.utils.data.DataLoader(train_data, batch_size = batch_size_train, shuffle=True, num_workers=2)

        # Run swarm.
        logger.info('Running CIFAR swarm..')
        try:
