...
import torch
from bittensor.v1.chain_data import AxonInfo, ChainDataType, DelegateInfo, NeuronInfo
...
def mock_from_scale_encoding(mocker):
    return mocker.patch("bittensor.v1.chain_data.from_scale_encoding")


@pytest.fixture
def mock_fix_decoded_values(mocker):
    return mocker.patch(
        "bittensor.v1.DelegateInfo.fix_decoded_values", side_effect=lambda x: x
    )
