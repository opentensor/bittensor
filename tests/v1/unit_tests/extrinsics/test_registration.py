...
from bittensor.v1.utils.registration import POWSolution
from bittensor.v1.extrinsics.registration import (
    MaxSuccessException,
...
def test_run_faucet_extrinsic_happy_path(
    ...
    with patch(
        "bittensor.v1.utils.registration._solve_for_difficulty_fast",
        return_value=mock_pow_solution,
    ) as mock_create_pow, patch("rich.prompt.Confirm.ask", return_value=True):
        from bittensor.v1.extrinsics.registration import run_faucet_extrinsic
...
def test_register_extrinsic_with_pow(
    ...
    with patch(
        ...
        "bittensor.v1.utils.registration._solve_for_difficulty_fast_cuda",
        return_value=mock_pow_solution if pow_success else None,
    ), patch.object(
        mock_subtensor,
        "_do_pow_register",
