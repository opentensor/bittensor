...
@pytest.mark.asyncio
async def test_submit_encrypted_extrinsic_success_with_revealed_execution(
    ...
    mocked_get_next_key = mocker.patch.object(
        ...
        new=mocker.AsyncMock(return_value=ml_kem_768_public_key),
    )
    ...
    type(mock_response.extrinsic_receipt).triggered_events = mocker.PropertyMock(
        return_value=mocker.AsyncMock(return_value=mock_triggered_events)()
    )

    mock_mev_extrinsic = mocker.MagicMock(spec=AsyncExtrinsicReceipt)
    mock_mev_extrinsic.is_success = mocker.AsyncMock(return_value=True)()
    mocked_wait_for_extrinsic = mocker.patch(
        "bittensor.core.extrinsics.asyncex.mev_shield.wait_for_extrinsic_by_hash",
        new=mocker.AsyncMock(return_value=mock_mev_extrinsic),
    )
    mocked_get_event_data = mocker.patch(
        "bittensor.core.extrinsics.asyncex.mev_shield.get_event_data_by_event_name",
        return_value={
            "module_id": "mevShield",
            "event_id": "EncryptedSubmitted",
            "attributes": {"id": shield_id},
...
@pytest.mark.asyncio
async def test_submit_encrypted_extrinsic_success_without_revealed_execution(
    subtensor, fake_wallet, mocker
):
...
@pytest.mark.asyncio
async def test_submit_encrypted_extrinsic_invalid_sign_with(
    subtensor, fake_wallet, mocker
):
...
@pytest.mark.asyncio
async def test_submit_encrypted_extrinsic_revealed_execution_without_inclusion(
    subtensor, fake_wallet, mocker
):
...
@pytest.mark.asyncio
async def test_submit_encrypted_extrinsic_unlock_failure(
    subtensor, fake_wallet, mocker
):
...
@pytest.mark.asyncio
async def test_submit_encrypted_extrinsic_next_key_not_available(
    subtensor, fake_wallet, mocker
):
    ...
    mocked_get_next_key = mocker.patch.object(
        subtensor, "get_mev_shield_next_key", new=mocker.AsyncMock(return_value=None)
    )
...
@pytest.mark.asyncio
async def test_submit_encrypted_extrinsic_encrypted_submitted_event_not_found(
    subtensor, fake_wallet, mocker
):
    ...
    mock_response.extrinsic_receipt = mocker.MagicMock()
    type(mock_response.extrinsic_receipt).triggered_events = mocker.PropertyMock(
        return_value=mocker.AsyncMock(return_value=[])()
    )
...
@pytest.mark.asyncio
async def test_submit_encrypted_extrinsic_failed_to_find_outcome(
    subtensor, fake_wallet, mocker
):
...
@pytest.mark.asyncio
async def test_submit_encrypted_extrinsic_execution_failure(
    subtensor, fake_wallet, mocker
):
    ...
    mock_mev_extrinsic = mocker.MagicMock(spec=AsyncExtrinsicReceipt)
    mock_mev_extrinsic.is_success = mocker.AsyncMock(return_value=False)()
    mock_mev_extrinsic.error_message = mocker.AsyncMock(return_value=error_message)()
    mocked_wait_for_extrinsic = mocker.patch(
        "bittensor.core.extrinsics.asyncex.mev_shield.wait_for_extrinsic_by_hash",
        new=mocker.AsyncMock(return_value=mock_mev_extrinsic),
    )
    ...
    mocked_format_error_message = mocker.patch(
        "bittensor.core.extrinsics.asyncex.mev_shield.format_error_message",
...
@pytest.mark.asyncio
async def test_submit_encrypted_extrinsic_sign_and_send_failure(
    subtensor, fake_wallet, mocker
):
...
@pytest.mark.asyncio
async def test_submit_encrypted_extrinsic_with_hotkey(subtensor, fake_wallet, mocker):
...
@pytest.mark.asyncio
async def test_submit_encrypted_extrinsic_with_period(subtensor, fake_wallet, mocker):
