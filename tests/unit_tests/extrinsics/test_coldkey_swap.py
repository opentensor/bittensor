...
from bittensor.core.extrinsics import coldkey_swap
from bittensor.core.extrinsics import sudo as sudo_extrinsics
from bittensor.core.extrinsics.pallets import SubtensorModule
from bittensor.core.types import ExtrinsicResponse


def test_announce_coldkey_swap_extrinsic(subtensor, mocker):
    """Verify that sync `announce_coldkey_swap_extrinsic` method calls proper methods."""
    # Preps
    ...
    mocked_pallet_instance.announce_coldkey_swap.return_value = mocker.MagicMock()
    ...
    # Call
    response = coldkey_swap.announce_coldkey_swap_extrinsic(
        subtensor=subtensor,
        wallet=wallet,
...
def test_announce_coldkey_swap_extrinsic_with_mev_protection(subtensor, mocker):
    """Verify that sync `announce_coldkey_swap_extrinsic` uses MEV protection when enabled."""
    # Preps
    wallet = mocker.MagicMock(spec=Wallet)
    wallet.coldkeypub.ss58_address = "5GrwvaEF5zXb26Fz9rcQpDWS57CtERHpNehXCPcNoHGKutQY"
    new_coldkey_ss58 = "5FHneW46xGXgs5mUiveU4sbTyGBzmstUspZC92UhjJM694ty"
    ...
    mocked_submit_encrypted.assert_called_once()
    assert response == mocked_submit_encrypted.return_value


def test_swap_coldkey_announced_extrinsic_success(subtensor, mocker):
    """Verify that sync `swap_coldkey_announced_extrinsic` method calls proper methods."""
    # Preps
    wallet = mocker.MagicMock(spec=Wallet)
    wallet.coldkeypub.ss58_address = "5GrwvaEF5zXb26Fz9rcQpDWS57CtERHpNehXCPcNoHGKutQY"
    new_coldkey_ss58 = "5FHneW46xGXgs5mUiveU4sbTyGBzmstUspZC92UhjJM694ty"
    ...
    mocked_pallet_instance = mocked_subtensor_module.return_value
    mocked_pallet_instance.swap_coldkey_announced.return_value = mocker.MagicMock()
    mocked_sign_and_send_extrinsic = mocker.patch.object(
        subtensor,
        "sign_and_send_extrinsic",
        return_value=ExtrinsicResponse(True, "Success"),
    ...
    # Call
    response = coldkey_swap.swap_coldkey_announced_extrinsic(
        subtensor=subtensor,
        wallet=wallet,
        new_coldkey_ss58=new_coldkey_ss58,
        mev_protection=False,
    ...
    # Asserts
    mocked_unlock_wallet.assert_called_once_with(wallet, False)
    mocked_get_announcement.assert_called_once_with(
    ...
    mocked_subtensor_module.assert_called_once_with(subtensor)
    mocked_pallet_instance.swap_coldkey_announced.assert_called_once_with(
...
def test_swap_coldkey_announced_extrinsic_no_announcement(subtensor, mocker):
    """Verify that sync `swap_coldkey_announced_extrinsic` returns error when no announcement."""
    # Preps
    wallet = mocker.MagicMock(spec=Wallet)
    wallet.coldkeypub.ss58_address = "5GrwvaEF5zXb26Fz9rcQpDWS57CtERHpNehXCPcNoHGKutQY"
    new_coldkey_ss58 = "5FHneW46xGXgs5mUiveU4sbTyGBzmstUspZC92UhjJM694ty"
...
def test_swap_coldkey_announced_extrinsic_hash_mismatch(subtensor, mocker):
    """Verify that sync `swap_coldkey_announced_extrinsic` returns error when hash doesn't match."""
    # Preps
    wallet = mocker.MagicMock(spec=Wallet)
    wallet.coldkeypub.ss58_address = "5GrwvaEF5zXb26Fz9rcQpDWS57CtERHpNehXCPcNoHGKutQY"
    new_coldkey_ss58 = "5FHneW46xGXgs5mUiveU4sbTyGBzmstUspZC92UhjJM694ty"
...
def test_swap_coldkey_announced_extrinsic_too_early(subtensor, mocker):
    """Verify that sync `swap_coldkey_announced_extrinsic` returns error when too early."""
    # Preps
    wallet = mocker.MagicMock(spec=Wallet)
    wallet.coldkeypub.ss58_address = "5GrwvaEF5zXb26Fz9rcQpDWS57CtERHpNehXCPcNoHGKutQY"
    new_coldkey_ss58 = "5FHneW46xGXgs5mUiveU4sbTyGBzmstUspZC92UhjJM694ty"
...
def test_dispute_coldkey_swap_extrinsic(subtensor, mocker):
    """Verify that sync dispute_coldkey_swap_extrinsic calls pallet and sign_and_send_extrinsic."""
    wallet = mocker.MagicMock(spec=Wallet)
    ...
    mocked_pallet_instance = mocked_subtensor_module.return_value
    mocked_pallet_instance.dispute_coldkey_swap.return_value = mocker.MagicMock()
    mocked_sign_and_send_extrinsic = mocker.patch.object(
        subtensor,
        "sign_and_send_extrinsic",
        return_value=ExtrinsicResponse(True, "Success"),
    ...
    # Call
    response = coldkey_swap.dispute_coldkey_swap_extrinsic(
        subtensor=subtensor,
        wallet=wallet,
        mev_protection=False,
    )
    ...
    mocked_unlock_wallet.assert_called_once_with(wallet, False)
    mocked_subtensor_module.assert_called_once_with(subtensor)
    mocked_pallet_instance.dispute_coldkey_swap.assert_called_once_with()
    mocked_sign_and_send_extrinsic.assert_called_once_with(
        call=mocked_pallet_instance.dispute_coldkey_swap.return_value,
        wallet=wallet,
        wait_for_inclusion=True,
        wait_for_finalization=True,
        period=None,
...
def test_reset_coldkey_swap_extrinsic(subtensor, mocker):
    ...
    mocked_sudo_call_extrinsic = mocker.patch.object(
        sudo_extrinsics,
        "sudo_call_extrinsic",
        return_value=ExtrinsicResponse(True, "Success"),
    )
    ...
    # Call
    response = sudo_extrinsics.reset_coldkey_swap_extrinsic(
        subtensor=subtensor,
        wallet=wallet,
    ...
    # Asserts
    mocked_sudo_call_extrinsic.assert_called_once_with(
        subtensor=subtensor,
        wallet=wallet,
    ...
    assert response == mocked_sudo_call_extrinsic.return_value


def test_swap_coldkey_extrinsic(subtensor, mocker):
    """Verify that sync swap_coldkey_extrinsic uses sudo_call_extrinsic."""
    wallet = mocker.MagicMock(spec=Wallet)
    old_coldkey_ss58 = "5GrwvaEF5zXb26Fz9rcQpDWS57CtERHpNehXCPcNoHGKutQY"
    new_coldkey_ss58 = "5FHneW46xGXgs5mUiveU4sbTyGBzmstUspZC92UhjJM694ty"
    swap_cost = 1000
    mocked_sudo_call_extrinsic = mocker.patch.object(
        sudo_extrinsics,
    ...
    # Call
    response = sudo_extrinsics.swap_coldkey_extrinsic(
        subtensor=subtensor,
        wallet=wallet,
        old_coldkey_ss58=old_coldkey_ss58,
        new_coldkey_ss58=new_coldkey_ss58,
        swap_cost=swap_cost,
    )
...
def test_subtensor_module_dispute_reset_swap_coldkey_call_names(subtensor, mocker):
    """Verify pallet call builders use correct runtime call names."""
    mocked_compose_call = mocker.patch.object(
        subtensor, "compose_call", return_value=mocker.MagicMock(spec=GenericCall)
    )
    pallet = SubtensorModule(subtensor)

    pallet.dispute_coldkey_swap()
