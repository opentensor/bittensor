...
def test_receptor_neuron_text():
    ...
    out, op, msg = receptor.forward( inputs = x, modality = bittensor.proto.Modality.TEXT)
...
def test_receptor_backward_neuron_text():
    ...
    out, op, msg = receptor.backward( inputs = x, grads = g, code = 0, modality = bittensor.proto.Modality.TEXT)
    assert op == bittensor.proto.ReturnCode.Unavailable
    assert list(out.shape) == [2, 4, bittensor.__network_dim__]
...
def test_receptor_neuron_backward_image():
    ...
    x = torch.tensor([ [ [ [ [ 1 ] ] ] ] ])
    g = torch.tensor([[1,2,3,4],[5,6,7,8]], dtype=torch.long)
    out, op, msg = receptor.backward( inputs = x, grads = g, code = 0, modality =bittensor.proto.Modality.IMAGE)
    assert op == bittensor.proto.ReturnCode.Unavailable
    assert list(out.shape) == [1, 1, bittensor.__network_dim__]
...
def test_receptor_neuron_backward_tensor():
    ...
    x = torch.rand(3, 3, bittensor.__network_dim__)
    g = torch.tensor([[1,2,3,4],[5,6,7,8]], dtype=torch.long)
    out, op, msg = receptor.backward( inputs = x, grads = g, code = 0, modality = bittensor.proto.Modality.TENSOR)
    assert op == bittensor.proto.ReturnCode.Unavailable
    assert list(out.shape) == [3, 3, bittensor.__network_dim__]
...
def test_receptor_backward_non_forward_success():
    ...
    x = torch.rand(3, 3, bittensor.__network_dim__)
    g = torch.tensor([[1,2,3,4],[5,6,7,8]], dtype=torch.long)
    out, op, msg = receptor.backward( inputs = x, grads = g, code = 1, modality = bittensor.proto.Modality.TENSOR)
    assert op == 1
    assert list(out.shape) == [3, 3, bittensor.__network_dim__]
