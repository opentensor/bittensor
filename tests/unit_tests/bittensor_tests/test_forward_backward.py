...
def test_dendrite_forward_text():
    ...
    dendrite.receptor_pool.forward = MagicMock(return_value = [ [torch.zeros([2, 4, bittensor.__network_dim__])], [0]]) 
    tensors, codes = dendrite.forward_text( endpoints=[endpoint], inputs=[x])
...
def test_dendrite_forward_tensor_pass_through_image():
    ...
    y = torch.zeros([3, 3, bittensor.__network_dim__])
    dendrite.receptor_pool.forward = MagicMock(return_value = [ [y, y, y] , [0, 0, 0]]) 
    tensors, codes = dendrite.forward_image( endpoints=[endpoint, endpoint, endpoint], inputs=[x, x, x])
    assert codes[0].item() == bittensor.proto.ReturnCode.Success
    assert codes[1].item() == bittensor.proto.ReturnCode.Success
    assert codes[2].item() == bittensor.proto.ReturnCode.Success
    assert tensors[0].shape == y.shape
...
def test_dendrite_forward_tensor_pass_through_tensor():
    ...
    y = torch.zeros([3, 3, bittensor.__network_dim__])
    dendrite.receptor_pool.forward = MagicMock(return_value = [ [y, y, y] , [0, 0, 0]]) 
    tensors, codes = dendrite.forward_tensor( endpoints = [endpoint, endpoint, endpoint], inputs=[x, x, x])
    assert codes[0].item() == bittensor.proto.ReturnCode.Success
    assert codes[1].item() == bittensor.proto.ReturnCode.Success
    assert codes[2].item() == bittensor.proto.ReturnCode.Success
    assert tensors[0].shape == y.shape
...
def test_dendrite_backward_multiple():
    ...
    dendrite.receptor_pool.forward = MagicMock(return_value = [ [y1, y2, y3], [0,0,0]]) 
    dendrite.receptor_pool.backward = MagicMock(return_value = [ [y1, y2, y3], [0,0,0]]) 
    tensors, codes = dendrite.forward_tensor( endpoints = [endpoint, endpoint, endpoint], inputs=[ x1, x2, x3 ])
    tensors[0].sum().backward()
