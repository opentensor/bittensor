...
with patch("importlib.metadata.version", return_value="8.0.0"):
    ...
    class MockAsyncSubtensor:
        ...
        async def get_account_nonce(self, addr):
            return 0
...
class TestTransactionOrchestrator(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        self.mock_subtensor.substrate.compose_call = AsyncMock(
            return_value="0xCallData"
        )
        self.mock_subtensor.substrate.create_signed_extrinsic = AsyncMock(
            return_value="0xSignedExtrinsic"
        )
        self.mock_subtensor.substrate.submit_extrinsic = AsyncMock(
            return_value="0xHash123"
        )
    ...
    async def test_submission_flow(self):
        ...
        self.mock_subtensor.substrate.compose_call.assert_called_with(
            call_module="Module", call_function="func", call_params={"arg": 1}
        )
    ...
    async def test_stuck_transaction(self):
        ...
        orch = TransactionOrchestrator(
            self.mock_wallet, self.mock_subtensor, config=config
        )
        await orch.start()
