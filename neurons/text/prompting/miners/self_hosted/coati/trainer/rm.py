...
class RewardModelTrainer(ABC):
    ...
    def __init__(
        ...
        if dist.is_initialized() and dist.get_world_size() > 1:
            train_sampler = DistributedSampler(
                train_dataset, shuffle=True, seed=42, drop_last=True
            )
        ...
        self.scheduler = lr_scheduler.CosineAnnealingLR(
            self.optimizer, self.train_dataloader.__len__() // 100
        )
    ...
    def fit(self):
        time = datetime.now()
        epoch_bar = tqdm(
            range(self.epochs), desc="Train epoch", disable=not is_rank_0()
        )
        for epoch in range(self.epochs):
            ...
            for chosen_ids, c_mask, reject_ids, r_mask in self.train_dataloader:
                ...
                if cnt == 100:
                    ...
                    if is_rank_0():
                        log = pd.DataFrame(
                            [[step_bar.n, loss.item(), dist, acc]],
                            columns=["step", "loss", "dist", "acc"],
                        )
                        log.to_csv(
                            "log_%s.csv" % time, mode="a", header=False, index=False
                        )
                step_bar.update()
