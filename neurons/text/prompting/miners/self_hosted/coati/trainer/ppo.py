...
class PPOTrainer(Trainer):
    ...
    def __init__(
        ...
        **generate_kwargs
        ...
        experience_maker = NaiveExperienceMaker(
            actor, critic, reward_model, initial_model, kl_coef
        )
        replay_buffer = NaiveReplayBuffer(
            train_batch_size, buffer_limit, buffer_cpu_offload
        )
    ...
    def training_step(self, experience: Experience) -> Dict[str, float]:
        ...
        action_log_probs = self.actor(
            experience.sequences, num_actions, attention_mask=experience.attention_mask
        )
        ...
        if self.ptx_coef != 0:
            ...
            attention_mask = next(iter(self.pretrain_dataloader))["attention_mask"].to(
                torch.cuda.current_device()
            )
            ptx_log_probs = self.actor.get_base_model()(
                ptx, attention_mask=attention_mask
            )["logits"][..., :-1, :]
...
def _set_default_generate_kwargs(
    strategy: Strategy, generate_kwargs: dict, actor: Actor
) -> None:
    origin_model = strategy._unwrap_actor(actor)
    new_kwargs = {**generate_kwargs}
    # use huggingface models method directly
    if "prepare_inputs_fn" not in generate_kwargs and hasattr(
...
def save_model(
    ...
    self.strategy.save_model(
        model=self.actor, path=path, only_rank0=only_rank0, tokenizer=tokenizer
    )
