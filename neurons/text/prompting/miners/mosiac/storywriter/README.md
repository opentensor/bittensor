...
## RoberMyers Miner
MosaicAI's MPT-7B Storywriter completion miner for bittensor's prompting network. 

# Example Usage
```
python3 -m pip install -r neurons/text/prompting/miners/mosaic/storywriter/requirements.txt
python3 neurons/text/prompting/miners/mosaic/storywriter/neuron.py
```

# Full Usage
```
usage: neuron.py [-h] [--mpt7B.model_name MPT7B.MODEL_NAME]
                 [--mpt7B.device MPT7B.DEVICE]
                 [--mpt7B.max_new_tokens MPT7B.MAX_NEW_TOKENS]
                 [--mpt7B.temperature MPT7B.TEMPERATURE]
                 [--mpt7B.greedy_sampling] [--mpt7B.do_prompt_injection]
                 [--mpt7B.system_prompt MPT7B.SYSTEM_PROMPT]
                 [--mpt7B.repetition-penalty MPT7B.REPETITION_PENALTY]
                 [--mpt7B.top_p MPT7B.TOP_P] [--mpt7B.top_k MPT7B.TOP_K]
                 ...
                 [--subtensor.register.cuda.use_cuda]
...
optional arguments:
  ...
  --mpt7B.model_name MPT7B.MODEL_NAME
  ...
  --mpt7B.greedy_sampling
                        Whether to use greedy sampling or not (if not, uses
                        multinomial sampling).
  --mpt7B.do_prompt_injection
                        Whether to use a custom "system" prompt instead of the
                        one sent by bittensor.
  --mpt7B.system_prompt MPT7B.SYSTEM_PROMPT
                        What prompt to replace the system prompt with
  --mpt7B.repetition-penalty MPT7B.REPETITION_PENALTY
                        Repetition penalty for greedy decoding. Between 1.0 and
                        infinity. 1.0 means no penalty. Default: 1.0
  --mpt7B.top_p MPT7B.TOP_P
                        Top-p (nucleus) sampling. Defaults to 1.0 (top-k
                        sampling). Must be between 0.0 and 1.0.
  --mpt7B.top_k MPT7B.TOP_K
                        Top-k sampling. Defaults to 0 (no top-k sampling). Must
  ...
  --neuron.blacklist.allow_non_registered
                        If True, the miner will allow non-registered hotkeys to
                        mine.
  --neuron.blacklist.default_stake NEURON.BLACKLIST.DEFAULT_STAKE
  ...
  --axon.external_port AXON.EXTERNAL_PORT
                        The public port this axon broadcasts to the network.
                        i.e. 8091
  --axon.external_ip AXON.EXTERNAL_IP
                        The external ip this axon broadcasts to the network to.
                        ie. [::]
  --axon.max_workers AXON.MAX_WORKERS
                        The maximum number connection handler threads working
                        simultaneously on this endpoint. The grpc server
                        distributes new worker threads to service requests up to
                        this number.
  --axon.maximum_concurrent_rpcs AXON.MAXIMUM_CONCURRENT_RPCS
                        Maximum number of allowed active connections
  --subtensor.network SUBTENSOR.NETWORK
                        The subtensor network flag. The likely choices are: --
                        finney (main network) -- local (local running network)
                        -- mock (creates a mock connection (for testing)) If
                        this option is set it overloads subtensor.chain_endpoint
                        with an entry point node from that network.
  --subtensor.chain_endpoint SUBTENSOR.CHAIN_ENDPOINT
                        The subtensor endpoint flag. If set, overrides the
                        --network flag.
  ...
  --subtensor.register.update_interval SUBTENSOR.REGISTER.UPDATE_INTERVAL, --subtensor.register.cuda.update_interval SUBTENSOR.REGISTER.UPDATE_INTERVAL, --cuda.update_interval SUBTENSOR.REGISTER.UPDATE_INTERVAL, -u SUBTENSOR.REGISTER.UPDATE_INTERVAL
                        The number of nonces to process before checking for next
                        block during registration
  --subtensor.register.no_output_in_place, --no_output_in_place
                        Whether to not ouput the registration statistics in-
                        place. Set flag to disable output in-place.
  --subtensor.register.verbose
  ...
  --subtensor.register.cuda.dev_id SUBTENSOR.REGISTER.CUDA.DEV_ID [SUBTENSOR.REGISTER.CUDA.DEV_ID ...], --cuda.dev_id SUBTENSOR.REGISTER.CUDA.DEV_ID [SUBTENSOR.REGISTER.CUDA.DEV_ID ...]
                        Set the CUDA device id(s). Goes by the order of speed.
                        (i.e. 0 is the fastest).
  ...
  --strict              If flagged, config will check that only exact arguemnts
                        have been set.
```