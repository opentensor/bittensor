## Stable_LM Miner
stabilityai/stablelm-tuned-alpha-7b Language Model Serving with BitTensor
This code is for running a language model powered by stabilityai through the BitTensor framework. 

# Example Usage
```
python3 -m pip install -r neurons/text/prompting/miners/stable_lm/requirements.txt
python3 neurons/text/prompting/miners/stable_lm/neuron.py
```

# Full Usage
```
usage: neuron.py [-h] [--stable_lm.model_name STABLE_LM.MODEL_NAME] [--stable_lm.device STABLE_LM.DEVICE] [--stable_lm.max_new_tokens STABLE_LM.MAX_NEW_TOKENS]
                 [--stable_lm.temperature STABLE_LM.TEMPERATURE] [--stable_lm.do_sample] [--netuid NETUID] [--neuron.name NEURON.NAME]
...
optional arguments:
  ...
  --stable_lm.device STABLE_LM.DEVICE
                        Device to load model
  --stable_lm.max_new_tokens STABLE_LM.MAX_NEW_TOKENS
                        Max tokens for model output.
  --stable_lm.temperature STABLE_LM.TEMPERATURE
                        Sampling temperature of model
  --stable_lm.do_sample    Whether to use sampling or not (if not, uses greedy decoding).
