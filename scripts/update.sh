...
linux_update_bittensor() {
    ohai "Updating bittensor@master into ~/.bittensor/bittensor"
    (cd ~/.bittensor/bittensor/ ; git checkout master ; git pull --ff-only ; git reset --hard ; git clean -xdf)
...
if [[ "$OS" == "Linux" ]]; then
    ...
    ohai "This script will update bittensor:"
